...
Tap numbered steps to toggle hits, then press **Play**. Stop before editing or changing tempo. **Starter beat** restores the default beat and tempo; **Clear** empties the grid after confirmation. The beat and tempo save automatically on this device, separately from MIDI session JSON exports. Drums and the MIDI piano sequencer can play together: tap **Done** to return to the piano sequencer without stopping the beat. Opening Drums also leaves MIDI playback running. Use **Stop drums** on the piano screen or **Stop** inside Drums to stop the beat independently. Leaving the foreground or an audio interruption stops drum audio, even when its screen is closed. The display stays awake while either sequencer plays. The two sequencers still have independent tempo and start controls; they are not clock-synchronized.
